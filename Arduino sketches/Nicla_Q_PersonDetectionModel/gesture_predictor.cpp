...
int PredictGesture(int8_t* output) {
  int max = -128;
  int max_index = 0;
  for (int i = 0; i < 1; i++) {  //changed from 4 to 2
    if(output[i]> max){
      max = output[i];
      max_index = i;
    }
  ...
  int max_predict_score = 0.0f;
  for (int i = 0; i < kGestureCount; i++) {
    int prediction_sum = 0.0f;
    for (int j = 0; j < kPredictionHistoryLength; ++j) {
      prediction_sum += prediction_history[i][j];
    }
  const int prediction_average_int = prediction_sum / kPredictionHistoryLength;
    const int8_t prediction_average = prediction_average_int;
